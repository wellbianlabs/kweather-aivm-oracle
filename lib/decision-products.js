// Decision-products engine for the K-Weather on-chain oracle.
//
// Each product is a pure function over an observation + recent history (the same
// fixed-point fields the oracle stores, already unscaled to human units):
//   o / history item = { timestamp, temperature(°C), humidity(%), precipitation(mm),
//     windSpeed(m/s), windDirection(°), pm10, pm25, solarRadiation(MJ/m²),
//     uvIndex, discomfortIndex }
//
// decide(latest, history) -> { signal, action, score(0..1), rationale, metrics }
// These are weather-driven business decisions that are genuinely computable from the
// oracle's data — built so an autonomous agent (or a contract) can act on them.

const r2 = (x) => Math.round(x * 100) / 100;
const mean = (a) => (a.length ? a.reduce((s, x) => s + x, 0) / a.length : 0);
const sumLast = (h, k, n) => h.slice(-n).reduce((s, o) => s + (o[k] || 0), 0);
const clamp01 = (x) => Math.max(0, Math.min(1, x));

// ── shared agent analytics: each product is an autonomous agent that PERCEIVES a time series,
// ANALYZES trend/accumulation, DECIDES with a confidence, and names the ACTion it would take. ──
const series = (h, k) => h.map((o) => o[k] || 0);
function slope(arr) { // least-squares trend per step (e.g. ℃ per hour)
  const n = arr.length; if (n < 2) return 0;
  const xm = (n - 1) / 2, ym = mean(arr);
  let num = 0, den = 0;
  for (let i = 0; i < n; i++) { num += (i - xm) * (arr[i] - ym); den += (i - xm) * (i - xm); }
  return den ? num / den : 0;
}
function trendOf(h, key = "temperature") {
  const s = slope(series(h, key));
  return { metric: key, perStep: r2(s), dir: s > 0.05 ? "rising" : s < -0.05 ? "falling" : "steady" };
}
// confidence from data sufficiency + how decisive the score is (clear high/low ⇒ more confident)
function confidenceOf(h, score) {
  const suff = Math.min(h.length, 24) / 24;
  const decisive = score >= 0.66 || score <= 0.2 ? 1 : score >= 0.5 ? 0.6 : 0.45;
  return r2(clamp01(0.4 + suff * 0.4 + decisive * 0.2));
}
// per-agent runtime spec: how the agent runs autonomously (role · cadence · on-chain action)
const AGENT_META = {
  "wind-dispatch": { role: "풍력 발전 급전 운영", cadence: "hourly", act: "출력 입찰량 자동 조정·차단 신호" },
  "crop-irrigation": { role: "스마트 관개 제어", cadence: "6h", act: "관개 밸브 ON/OFF 스케줄" },
  "flood-watch": { role: "침수·집중호우 감시", cadence: "hourly", act: "파라메트릭 보험 트리거·대피 경보" },
  "heat-demand-response": { role: "냉방 수요반응 운영", cadence: "hourly", act: "ESS 방전·피크저감 발동" },
  "cold-chain": { role: "콜드체인 운송 감시", cadence: "hourly", act: "경로 재배정·냉방 강화 지시" },
  "construction-safety": { role: "건설 현장 안전 관제", cadence: "hourly", act: "고소·크레인 작업 중단 게이트" },
  "event-hedge": { role: "옥외 행사 날씨 헤지", cadence: "3h", act: "날씨 파생 헤지·연기 결정" },
  "tourism-comfort": { role: "관광 동적 가격", cadence: "3h", act: "숙박·입장 가격 자동 조정" },
  "wildfire-risk": { role: "산불 기상 감시", cadence: "3h", act: "통제·순찰 경보·보험 트리거" },
  "heat-stress": { role: "온열·한랭 작업안전", cadence: "hourly", act: "옥외작업 중단·휴식주기 단축" },
  "storm-pressure": { role: "폭풍·저기압 감시", cadence: "hourly", act: "운항 통제·기상 경보" },
  "visibility-ops": { role: "저시정 운항 관제", cadence: "hourly", act: "지연·서행·운항 보류" },
  "snow-ops": { role: "제설 자원 배치", cadence: "hourly", act: "제설차 출동·도로 통제" },
  "air-quality-ops": { role: "대기질 작업 게이트", cadence: "hourly", act: "실외작업 제한·마스크 의무" },
  "marine-ops": { role: "해상·어선 운항", cadence: "hourly", act: "출항 통제·소형선 유의보" },
  "drone-ops": { role: "드론·UAV 배송 윈도우", cadence: "30m", act: "비행 허가/제한/금지" },
  "road-frost": { role: "도로 결빙 감시", cadence: "hourly", act: "염화칼슘 살포·서행 경보" },
  "autonomous-driving": { role: "자율주행 운행 가능성", cadence: "15m", act: "FULL/DEGRADED/DISENGAGE 핸드오버" },
  "heating-demand": { role: "난방 수요 예측", cadence: "hourly", act: "가스·전력 수급 사전 확보" },
  "crop-disease": { role: "작물 병해 예찰", cadence: "6h", act: "살균제 방제 적기 알림" },
  "frost-alert": { role: "작물 서리 경보", cadence: "hourly", act: "방상팬·살수·피복 발동" },
  "retail-footfall": { role: "리테일 유동인구·인력", cadence: "3h", act: "인력 증감·배달 강화" },
  "vector-risk": { role: "모기 매개 방역", cadence: "6h", act: "유문등·방제 자원 배치" },
  "powerline-icing": { role: "송전 착빙 감시", cadence: "hourly", act: "순시·제빙 출동 경보" },
  "water-demand": { role: "상수도 수요 예측", cadence: "3h", act: "배수지·펌프 증설 사전 대응" },
};

const PRODUCTS = [
  {
    id: "wind-dispatch", name: "Wind Farm Dispatch", emoji: "🌬️", sector: "Energy",
    inputs: ["windSpeed", "history"],
    decide(o, h) {
      const w = mean(h.map((x) => x.windSpeed));
      const cutIn = 3, rated = 12, cutOut = 25;
      const kw = w < cutIn || w >= cutOut ? 0 : w >= rated ? 500 : 500 * Math.pow((w - cutIn) / (rated - cutIn), 3);
      const cf = clamp01(kw / 500);
      const storm = w >= cutOut;
      const signal = storm ? "CUTOUT" : cf >= 0.6 ? "HIGH" : cf >= 0.25 ? "MODERATE" : "LOW";
      const action = storm ? "CURTAIL (강풍 차단 — 안전 정지)" : cf >= 0.6 ? "DISPATCH (최대 송전)" : cf >= 0.25 ? "PARTIAL (부분 가동)" : "STANDBY (대기)";
      return { signal, action, score: r2(cf), rationale: `평균 풍속 ${r2(w)} m/s → 추정 출력 ${Math.round(kw)} kW${storm ? " (컷아웃 25 m/s 초과)" : ""}.`, metrics: { avgWind: r2(w), estKw: Math.round(kw), capacityFactor: r2(cf) } };
    },
  },
  {
    id: "crop-irrigation", name: "Smart Irrigation (Agritech)", emoji: "🌾", sector: "Agriculture",
    inputs: ["precipitation", "temperature", "humidity", "history"],
    decide(o, h) {
      const p24 = sumLast(h, "precipitation", 24);
      const p72 = sumLast(h, "precipitation", 72);
      const et0 = Math.max(0, (o.temperature - 5) * 0.2 + (o.solarRadiation || 0) * 0.1 + (100 - o.humidity) * 0.02); // crude ET proxy (mm/day)
      const deficit = r2(et0 * 1 - p24);
      let signal, action;
      if (p72 < 3 && o.temperature > 28) { signal = "DROUGHT"; action = "IRRIGATE_NOW + 가뭄 경보"; }
      else if (deficit > 2) { signal = "DRY"; action = "IRRIGATE_NOW (관개 실행)"; }
      else if (p24 >= 10) { signal = "WET"; action = "SKIP (관개 중단 — 충분한 강수)"; }
      else { signal = "OK"; action = "SCHEDULE (예정대로/소량)"; }
      return { signal, action, score: clamp01(deficit / 5), rationale: `24h 강수 ${r2(p24)}mm, 증발산 추정 ${r2(et0)}mm → 수분수지 ${deficit}mm (72h 강수 ${r2(p72)}mm).`, metrics: { precip24h: r2(p24), precip72h: r2(p72), et0: r2(et0), waterDeficit: deficit } };
    },
  },
  {
    id: "flood-watch", name: "Flood / Heavy-Rain Watch", emoji: "🌊", sector: "Insurance / Public Safety",
    inputs: ["precipitation", "history"],
    decide(o, h) {
      const p1 = o.precipitation || 0;
      const p6 = sumLast(h, "precipitation", 6);
      const p24 = sumLast(h, "precipitation", 24);
      let signal, action, score;
      if (p6 >= 30 || p24 >= 80) { signal = "WARNING"; action = "EVACUATE_PREP (대피·펌프 가동 / 파라메트릭 지급 트리거)"; score = 1; }
      else if (p6 >= 15 || p24 >= 40) { signal = "WATCH"; action = "MONITOR (배수 점검·경계)"; score = 0.6; }
      else { signal = "CLEAR"; action = "CLEAR (정상)"; score = clamp01(p24 / 80); }
      return { signal, action, score: r2(score), rationale: `시간당 ${r2(p1)}mm · 6h ${r2(p6)}mm · 24h ${r2(p24)}mm 누적.`, metrics: { precip1h: r2(p1), precip6h: r2(p6), precip24h: r2(p24) } };
    },
  },
  {
    id: "heat-demand-response", name: "Cooling Demand Response (Grid)", emoji: "🔥", sector: "Energy / Utilities",
    inputs: ["temperature", "discomfortIndex"],
    decide(o) {
      const t = o.temperature, di = o.discomfortIndex;
      const cdd = Math.max(0, t - 24); // cooling degrees over base 24°C
      let signal, action, score;
      if (t >= 35 || di >= 80) { signal = "EMERGENCY"; action = "EMERGENCY_DR (긴급 수요반응 발동)"; score = 1; }
      else if (t >= 32 || di >= 75) { signal = "PEAK"; action = "PEAK_SHAVE (피크 저감·ESS 방전)"; score = 0.7; }
      else { signal = "NORMAL"; action = "NORMAL (정상 운영)"; score = clamp01(cdd / 11); }
      return { signal, action, score: r2(score), rationale: `기온 ${r2(t)}℃ · 불쾌지수 ${r2(di)} → 냉방도일 ${r2(cdd)}.`, metrics: { temperature: r2(t), discomfortIndex: r2(di), coolingDegrees: r2(cdd) } };
    },
  },
  {
    id: "cold-chain", name: "Cold-Chain Logistics Risk", emoji: "🧊", sector: "Logistics",
    inputs: ["temperature"],
    decide(o) {
      const t = o.temperature;
      let signal, action, score;
      if (t >= 35) { signal = "HEAT_RISK"; action = "REROUTE/REFRIGERATE (적재 냉방 강화·경로 변경)"; score = 1; }
      else if (t >= 30) { signal = "MONITOR"; action = "MONITOR (온도 모니터링 강화)"; score = 0.6; }
      else if (t <= 0) { signal = "FREEZE_RISK"; action = "FREEZE_GUARD (동결 방지)"; score = 0.6; }
      else { signal = "OK"; action = "OK (정상 운송)"; score = clamp01((t - 20) / 15); }
      return { signal, action, score: r2(score), rationale: `외기 ${r2(t)}℃ — 신선/냉장 화물 ${t >= 30 ? "고온 변질 위험" : t <= 0 ? "동결 위험" : "정상"}.`, metrics: { temperature: r2(t) } };
    },
  },
  {
    id: "construction-safety", name: "Construction Site Safety", emoji: "🏗️", sector: "Construction / HSE",
    inputs: ["windSpeed", "precipitation", "temperature"],
    decide(o) {
      const w = o.windSpeed, p = o.precipitation || 0, t = o.temperature;
      let signal, action, score;
      if (w >= 15 || p >= 10) { signal = "STOP"; action = "STOP (크레인·고소작업 중단)"; score = 1; }
      else if (w >= 10 || p >= 2 || t >= 35) { signal = "CAUTION"; action = "CAUTION (작업 제한·폭염/강풍 주의)"; score = 0.6; }
      else { signal = "WORK"; action = "WORK (정상 작업)"; score = clamp01(Math.max(w / 15, p / 10)); }
      return { signal, action, score: r2(score), rationale: `풍속 ${r2(w)} m/s · 시간당 강수 ${r2(p)}mm · 기온 ${r2(t)}℃.`, metrics: { windSpeed: r2(w), precip1h: r2(p), temperature: r2(t) } };
    },
  },
  {
    id: "event-hedge", name: "Outdoor Event Weather Hedge", emoji: "🎪", sector: "Insurance / Events",
    inputs: ["precipitation", "windSpeed", "discomfortIndex", "history"],
    decide(o, h) {
      const p24 = sumLast(h, "precipitation", 24);
      const w = o.windSpeed, di = o.discomfortIndex, p = o.precipitation || 0;
      let signal, action, score;
      if (p >= 5 || p24 >= 20 || w >= 14) { signal = "ADVERSE"; action = "POSTPONE/PAYOUT (연기 또는 파라메트릭 지급)"; score = 1; }
      else if (p24 >= 5 || w >= 9 || di >= 80) { signal = "RISK"; action = "HEDGE (날씨 헤지·우천 대비)"; score = 0.6; }
      else { signal = "FAVORABLE"; action = "CONFIRM (정상 개최)"; score = clamp01(Math.max(p24 / 20, w / 14)); }
      return { signal, action, score: r2(score), rationale: `현재 강수 ${r2(p)}mm/h · 24h ${r2(p24)}mm · 풍속 ${r2(w)} m/s · 불쾌지수 ${r2(di)}.`, metrics: { precip1h: r2(p), precip24h: r2(p24), windSpeed: r2(w), discomfortIndex: r2(di) } };
    },
  },
  {
    id: "tourism-comfort", name: "Tourism Comfort & Dynamic Pricing", emoji: "🏖️", sector: "Travel / Retail",
    inputs: ["temperature", "discomfortIndex", "precipitation"],
    decide(o) {
      const t = o.temperature, di = o.discomfortIndex, p = o.precipitation || 0;
      // comfort 100 best: penalize temp away from 23°C, high discomfort, rain
      let c = 100 - Math.abs(t - 23) * 3 - Math.max(0, di - 70) * 2 - Math.min(40, p * 8);
      c = Math.max(0, Math.min(100, Math.round(c)));
      const signal = c >= 70 ? "EXCELLENT" : c >= 45 ? "FAIR" : "POOR";
      const action = c >= 70 ? "PEAK_PRICING (성수 가격·프로모션)" : c >= 45 ? "NORMAL (정상 가격)" : "DISCOUNT (실내대안·할인)";
      return { signal, action, score: r2(c / 100), rationale: `쾌적도 ${c}/100 (기온 ${r2(t)}℃·불쾌 ${r2(di)}·강수 ${r2(p)}mm).`, metrics: { comfortScore: c, temperature: r2(t), discomfortIndex: r2(di) } };
    },
  },
  {
    id: "wildfire-risk", name: "Wildfire / Fire-Weather Risk", emoji: "🔥", sector: "Insurance / Public Safety",
    inputs: ["humidity", "temperature", "windSpeed", "precipitation", "history"],
    decide(o, h) {
      const rh = o.humidity, t = o.temperature, w = o.windSpeed;
      const p168 = sumLast(h, "precipitation", Math.min(168, h.length));
      // simple fire-weather proxy
      const dry = rh < 30 && p168 < 5;
      let signal, action, score;
      if (dry && t > 30 && w > 5) { signal = "EXTREME"; action = "RED_FLAG (화재경보·통제 / 파라메트릭 트리거)"; score = 1; }
      else if (rh < 40 && w > 3 && p168 < 10) { signal = "ELEVATED"; action = "ELEVATED (예방순찰·발화원 관리)"; score = 0.6; }
      else { signal = "LOW"; action = "LOW (정상)"; score = clamp01((40 - rh) / 40); }
      return { signal, action, score: r2(score), rationale: `습도 ${r2(rh)}% · 기온 ${r2(t)}℃ · 풍속 ${r2(w)} m/s · 7일 강수 ${r2(p168)}mm.`, metrics: { humidity: r2(rh), temperature: r2(t), windSpeed: r2(w), precip7d: r2(p168) } };
    },
  },
  {
    id: "heat-stress", name: "Heat / Cold Stress (Felt Temp)", emoji: "🌡️", sector: "HSE / Health",
    inputs: ["senseTemp"],
    decide(o) {
      const st = o.senseTemp != null ? o.senseTemp : o.temperature;
      let signal, action, score;
      if (st >= 38) { signal = "EXTREME_HEAT"; action = "STOP (옥외작업 중단·온열질환 경보)"; score = 1; }
      else if (st >= 33) { signal = "HIGH_HEAT"; action = "LIMIT (휴식주기 단축·수분공급)"; score = 0.7; }
      else if (st <= -10) { signal = "EXTREME_COLD"; action = "COLD_GUARD (한랭질환 경보)"; score = 1; }
      else if (st <= 0) { signal = "COLD"; action = "WARM (방한·동상 주의)"; score = 0.6; }
      else { signal = "NORMAL"; action = "OK (정상)"; score = clamp01((st - 26) / 12); }
      return { signal, action, score: r2(score), rationale: `체감온도 ${r2(st)}℃ (기온 ${r2(o.temperature)}℃).`, metrics: { senseTemp: r2(st), airTemp: r2(o.temperature) } };
    },
  },
  {
    id: "storm-pressure", name: "Storm / Low-Pressure Watch", emoji: "🌀", sector: "Insurance / Marine",
    inputs: ["pressure", "windSpeed"],
    decide(o) {
      const pa = o.pressure || 0, w = o.windSpeed;
      let signal, action, score;
      if ((pa > 0 && pa < 990) || w >= 17) { signal = "STORM"; action = "ALERT (폭풍·저기압 경보 / 운항 통제)"; score = 1; }
      else if ((pa > 0 && pa < 1000) || w >= 11) { signal = "UNSETTLED"; action = "WATCH (기상 악화 대비)"; score = 0.6; }
      else { signal = "STABLE"; action = "OK (안정)"; score = pa ? clamp01((1013 - pa) / 23) : clamp01(w / 17); }
      return { signal, action, score: r2(score), rationale: `기압 ${r2(pa)}hPa · 풍속 ${r2(w)} m/s.`, metrics: { pressure: r2(pa), windSpeed: r2(w) } };
    },
  },
  {
    id: "visibility-ops", name: "Visibility Operations (Aviation/Road/Marine)", emoji: "🌫️", sector: "Transport",
    inputs: ["visibility"],
    decide(o) {
      const v = o.visibility || 0;
      let signal, action, score;
      if (v > 0 && v < 1000) { signal = "FOG"; action = "HALT/DELAY (저시정 — 운항 지연·서행)"; score = 1; }
      else if (v > 0 && v < 4000) { signal = "LOW"; action = "CAUTION (감속·등화 점등)"; score = 0.6; }
      else { signal = "CLEAR"; action = "GO (정상)"; score = v ? clamp01((6000 - v) / 6000) : 0; }
      return { signal, action, score: r2(score), rationale: `가시거리 ${v} m.`, metrics: { visibility: v } };
    },
  },
  {
    id: "snow-ops", name: "Snow Removal / Winter Logistics", emoji: "❄️", sector: "Logistics / Public",
    inputs: ["snowfall", "temperature"],
    decide(o) {
      const s = o.snowfall || 0, t = o.temperature;
      let signal, action, score;
      if (s >= 5) { signal = "HEAVY_SNOW"; action = "DEPLOY (제설차 투입·도로 통제)"; score = 1; }
      else if (s >= 1) { signal = "SNOW"; action = "PREP (제설 준비·염화칼슘 살포)"; score = 0.6; }
      else if (t <= 0) { signal = "FREEZE"; action = "ANTI_ICE (결빙 대비)"; score = 0.4; }
      else { signal = "CLEAR"; action = "OK (정상)"; score = clamp01(s / 5); }
      return { signal, action, score: r2(score), rationale: `적설 ${r2(s)}cm · 기온 ${r2(t)}℃.`, metrics: { snowfall: r2(s), temperature: r2(t) } };
    },
  },
  {
    id: "air-quality-ops", name: "Air-Quality Operations Gate", emoji: "😷", sector: "HSE / Logistics (KR)",
    inputs: ["pm25", "pm10"],
    decide(o) {
      const pm25 = o.pm25 || 0, pm10 = o.pm10 || 0;
      let signal, action, score;
      if (pm25 >= 75 || pm10 >= 150) { signal = "HAZARD"; action = "HALT (실외작업 중단·마스크 의무)"; score = 1; }
      else if (pm25 >= 35 || pm10 >= 80) { signal = "UNHEALTHY"; action = "LIMIT (노출 제한·민감군 보호)"; score = 0.6; }
      else { signal = "GOOD"; action = "GO (정상)"; score = clamp01(pm25 / 75); }
      return { signal, action, score: r2(score), rationale: `PM2.5 ${pm25} ㎍/㎥ · PM10 ${pm10} ㎍/㎥.`, metrics: { pm25, pm10 } };
    },
  },
  {
    id: "marine-ops", name: "Marine Operations (Fishing/Shipping)", emoji: "⚓", sector: "Marine",
    inputs: ["windSpeed", "visibility", "pressure"],
    decide(o) {
      const w = o.windSpeed, v = o.visibility || 0, pa = o.pressure || 0;
      let signal, action, score;
      if (w >= 14 || (v > 0 && v < 1000) || (pa > 0 && pa < 995)) { signal = "PORT_HOLD"; action = "출항 통제 (높은 파고·저시정·저기압)"; score = 1; }
      else if (w >= 9 || (v > 0 && v < 2000)) { signal = "CAUTION"; action = "주의 출항 (소형선 유의)"; score = 0.6; }
      else { signal = "SAIL"; action = "출항 양호"; score = clamp01(w / 14); }
      return { signal, action, score: r2(score), rationale: `풍속 ${r2(w)} m/s · 가시거리 ${v} m · 기압 ${r2(pa)}hPa.`, metrics: { windSpeed: r2(w), visibility: v, pressure: r2(pa) } };
    },
  },
  {
    id: "drone-ops", name: "Drone / UAV Delivery Window", emoji: "🚁", sector: "UAV / Delivery",
    inputs: ["windSpeed", "precipitation", "visibility"],
    decide(o) {
      const w = o.windSpeed, p = o.precipitation || 0, v = o.visibility || 0;
      let signal, action, score;
      if (w >= 12 || p >= 2 || (v > 0 && v < 1500)) { signal = "NO_FLY"; action = "비행 금지 (강풍·강수·저시정)"; score = 1; }
      else if (w >= 8 || p > 0) { signal = "RESTRICT"; action = "제한 비행 (숙련 조종·단거리)"; score = 0.6; }
      else { signal = "FLY"; action = "비행 가능"; score = clamp01(w / 12); }
      return { signal, action, score: r2(score), rationale: `풍속 ${r2(w)} m/s · 강수 ${r2(p)}mm · 가시거리 ${v} m.`, metrics: { windSpeed: r2(w), precip: r2(p), visibility: v } };
    },
  },
  {
    id: "road-frost", name: "Road Frost / Black Ice", emoji: "🛣️", sector: "Transport / Safety",
    inputs: ["temperature", "precipitation", "humidity"],
    decide(o) {
      const tt = o.temperature, p = o.precipitation || 0, rh = o.humidity;
      let signal, action, score;
      if (tt <= 0 && (p > 0 || rh >= 85)) { signal = "ICE"; action = "결빙 위험 (제설·염화칼슘 살포)"; score = 1; }
      else if (tt <= 2 && rh >= 80) { signal = "WATCH"; action = "살얼음 주의 (감속·예비 살포)"; score = 0.6; }
      else { signal = "CLEAR"; action = "정상"; score = clamp01((4 - tt) / 4); }
      return { signal, action, score: r2(score), rationale: `기온 ${r2(tt)}℃ · 강수 ${r2(p)}mm · 습도 ${rh}%.`, metrics: { temperature: r2(tt), precip: r2(p), humidity: rh } };
    },
  },
  {
    id: "autonomous-driving", name: "Autonomous Driving (AV/ADAS) Operating Window", emoji: "🚗", sector: "Mobility / Transport",
    inputs: ["visibility", "precipitation", "snowfall", "temperature", "windSpeed"],
    decide(o) {
      const v = o.visibility || 0, p = o.precipitation || 0, s = o.snowfall || 0, tt = o.temperature, w = o.windSpeed;
      const blackIce = tt <= 0 && (p > 0 || s > 0); // 노면 결빙
      let signal, action, score;
      if ((v > 0 && v < 1000) || p >= 8 || s >= 3 || blackIce) {
        signal = "DISENGAGE"; action = "수동 전환·운행 보류 (저시정·폭우·적설·노면결빙)"; score = 1;
      } else if ((v > 0 && v < 4000) || p >= 2 || s > 0 || w >= 14 || tt <= 2) {
        signal = "DEGRADED"; action = "제한 자율주행 (속도 하향·차간 확대·핸드오버 대비)"; score = 0.6;
      } else {
        signal = "FULL"; action = "자율주행 정상"; score = clamp01(Math.max(p / 8, s / 3, w / 14));
      }
      return { signal, action, score: r2(score), rationale: `가시거리 ${v} m · 강수 ${r2(p)}mm · 적설 ${r2(s)}cm · 기온 ${r2(tt)}℃ · 풍속 ${r2(w)} m/s.`, metrics: { visibility: v, precip: r2(p), snowfall: r2(s), temperature: r2(tt), windSpeed: r2(w) } };
    },
  },
  {
    id: "heating-demand", name: "Heating Demand (Grid/Gas)", emoji: "🔥", sector: "Utilities",
    inputs: ["temperature", "senseTemp"],
    decide(o) {
      const tt = o.senseTemp != null ? o.senseTemp : o.temperature, hdd = Math.max(0, 18 - tt);
      let signal, action, score;
      if (tt <= -5) { signal = "PEAK"; action = "난방 피크 (가스·전력 수급 대비)"; score = 1; }
      else if (tt <= 5) { signal = "HIGH"; action = "높은 난방 수요"; score = 0.7; }
      else { signal = "NORMAL"; action = "정상"; score = clamp01(hdd / 23); }
      return { signal, action, score: r2(score), rationale: `체감 ${r2(tt)}℃ → 난방도일 ${r2(hdd)}.`, metrics: { senseTemp: r2(tt), heatingDegrees: r2(hdd) } };
    },
  },
  {
    id: "crop-disease", name: "Crop Disease Pressure (Fungal)", emoji: "🍄", sector: "Agriculture",
    inputs: ["temperature", "humidity", "history"],
    decide(o, h) {
      const rh = o.humidity, tt = o.temperature, wetHours = h.filter((x) => x.humidity >= 90).length;
      let signal, action, score;
      if (rh >= 90 && tt >= 15 && tt <= 25) { signal = "HIGH"; action = "방제 권고 (살균제 살포 적기)"; score = 1; }
      else if (rh >= 80 && tt >= 10) { signal = "ELEVATED"; action = "예찰 강화"; score = 0.6; }
      else { signal = "LOW"; action = "정상"; score = clamp01(rh / 100); }
      return { signal, action, score: r2(score), rationale: `습도 ${rh}% · 기온 ${r2(tt)}℃ · 다습시간 ${wetHours}h.`, metrics: { humidity: rh, temperature: r2(tt), wetHours } };
    },
  },
  {
    id: "frost-alert", name: "Crop Frost Alert", emoji: "🌱", sector: "Agriculture",
    inputs: ["temperature", "humidity", "windSpeed"],
    decide(o) {
      const tt = o.temperature, rh = o.humidity, w = o.windSpeed;
      let signal, action, score;
      if (tt <= 0) { signal = "FROST"; action = "서리 경보 (방상팬·살수·피복)"; score = 1; }
      else if (tt <= 3 && w < 2 && rh < 70) { signal = "RISK"; action = "복사냉각 서리 주의"; score = 0.6; }
      else { signal = "SAFE"; action = "정상"; score = clamp01((4 - tt) / 4); }
      return { signal, action, score: r2(score), rationale: `기온 ${r2(tt)}℃ · 습도 ${rh}% · 풍속 ${r2(w)} m/s.`, metrics: { temperature: r2(tt), humidity: rh, windSpeed: r2(w) } };
    },
  },
  {
    id: "retail-footfall", name: "Retail Footfall & Staffing", emoji: "🛍️", sector: "Retail",
    inputs: ["precipitation", "temperature", "discomfortIndex"],
    decide(o) {
      const p = o.precipitation || 0, tt = o.temperature, di = o.discomfortIndex;
      let signal, action, score;
      if (p >= 3 || di >= 80 || tt <= 0) { signal = "LOW"; action = "방문 감소 → 인력 축소·배달 강화"; score = 1; }
      else if (p > 0 || di >= 75) { signal = "SOFT"; action = "다소 감소"; score = 0.5; }
      else { signal = "GOOD"; action = "방문 양호 → 정상/증원"; score = clamp01((20 - Math.abs(tt - 22)) / 20); }
      return { signal, action, score: r2(score), rationale: `강수 ${r2(p)}mm · 기온 ${r2(tt)}℃ · 불쾌 ${r2(di)}.`, metrics: { precip: r2(p), temperature: r2(tt), discomfortIndex: r2(di) } };
    },
  },
  {
    id: "vector-risk", name: "Mosquito / Vector Activity", emoji: "🦟", sector: "Public Health",
    inputs: ["temperature", "humidity", "precipitation", "history"],
    decide(o, h) {
      const tt = o.temperature, rh = o.humidity, p72 = sumLast(h, "precipitation", Math.min(72, h.length));
      let signal, action, score;
      if (tt >= 25 && rh >= 70 && p72 >= 5) { signal = "HIGH"; action = "방역 강화 (유문등·방제)"; score = 1; }
      else if (tt >= 20 && rh >= 60) { signal = "ELEVATED"; action = "모니터링"; score = 0.6; }
      else { signal = "LOW"; action = "정상"; score = clamp01((tt - 15) / 15); }
      return { signal, action, score: r2(score), rationale: `기온 ${r2(tt)}℃ · 습도 ${rh}% · 72h 강수 ${r2(p72)}mm.`, metrics: { temperature: r2(tt), humidity: rh, precip72h: r2(p72) } };
    },
  },
  {
    id: "powerline-icing", name: "Power-line Icing Risk", emoji: "🧊", sector: "Grid / Utilities",
    inputs: ["snowfall", "temperature", "windSpeed"],
    decide(o) {
      const s = o.snowfall || 0, tt = o.temperature, w = o.windSpeed;
      let signal, action, score;
      if ((s >= 1 && tt <= 0) || (tt >= -3 && tt <= 1 && w >= 5 && s > 0)) { signal = "ICING"; action = "착빙 경보 (순시·제빙 대비)"; score = 1; }
      else if (tt <= 2 && s > 0) { signal = "WATCH"; action = "착빙 주의"; score = 0.6; }
      else { signal = "LOW"; action = "정상"; score = clamp01(s / 3); }
      return { signal, action, score: r2(score), rationale: `적설 ${r2(s)}cm · 기온 ${r2(tt)}℃ · 풍속 ${r2(w)} m/s.`, metrics: { snowfall: r2(s), temperature: r2(tt), windSpeed: r2(w) } };
    },
  },
  {
    id: "water-demand", name: "Water Demand Forecast", emoji: "💧", sector: "Utilities",
    inputs: ["temperature", "discomfortIndex"],
    decide(o) {
      const tt = o.temperature, di = o.discomfortIndex;
      let signal, action, score;
      if (tt >= 33 || di >= 80) { signal = "PEAK"; action = "급수 피크 (배수지·펌프 증설)"; score = 1; }
      else if (tt >= 28) { signal = "HIGH"; action = "높은 수요"; score = 0.6; }
      else { signal = "NORMAL"; action = "정상"; score = clamp01((tt - 20) / 13); }
      return { signal, action, score: r2(score), rationale: `기온 ${r2(tt)}℃ · 불쾌 ${r2(di)}.`, metrics: { temperature: r2(tt), discomfortIndex: r2(di) } };
    },
  },
];

const BY_ID = new Map(PRODUCTS.map((p) => [p.id, p]));
const metaOf = (id) => AGENT_META[id] || { role: "", cadence: "hourly", act: "" };

function list() {
  return PRODUCTS.map((p) => { const m = metaOf(p.id); return { id: p.id, name: p.name, emoji: p.emoji, sector: p.sector, inputs: p.inputs, role: m.role, cadence: m.cadence, action: m.act }; });
}
function get(id) { return BY_ID.get(String(id)); }
// run one agent: perceive (latest + history) → analyze → decide, enriched with confidence + trend + runtime
function decide(id, latest, history) {
  const p = BY_ID.get(String(id));
  if (!p) return null;
  const h = history || [];
  const out = p.decide(latest, h);
  const m = metaOf(p.id);
  return {
    product: p.id, name: p.name, emoji: p.emoji, sector: p.sector, ...out,
    confidence: confidenceOf(h, out.score), trend: trendOf(h), samples: h.length,
    agent: { role: m.role, cadence: m.cadence, action: m.act },
  };
}
function decideAll(latest, history) {
  return PRODUCTS.map((p) => decide(p.id, latest, history));
}

module.exports = { PRODUCTS, AGENT_META, list, get, decide, decideAll };
