...
const PRODUCTS = [
  {
    id: "solar-yield", name: "Solar PV Yield & Power Trading", emoji: "⚡", sector: "Energy",
    inputs: ["solarRadiation", "history"],
    decide(o, h) {
      const day = h.filter((x) => x.solarRadiation > 0);
      const ss = day.length ? day : h;
      const avgSolar = mean(ss.map((x) => x.solarRadiation));
      const estKw = Math.min(1000, avgSolar * 0.2778 * 5000 * 0.2); // MJ/m²->kWh/m² * area * efficiency
      const cf = clamp01(estKw / 1000);
      const signal = cf >= 0.6 ? "HIGH" : cf >= 0.3 ? "MODERATE" : "LOW";
      const action = cf >= 0.6 ? "SELL_POWER (PPA/스팟 판매)" : cf >= 0.3 ? "HOLD (포지션 유지)" : "BUY_HEDGE (DeFi 파생 헤지)";
      return { signal, action, score: r2(cf), rationale: `평균 일사량 ${r2(avgSolar)} MJ/m² → 추정 출력 ${Math.round(estKw)} kW (용량계수 ${Math.round(cf * 100)}%).`, metrics: { avgSolar: r2(avgSolar), estKw: Math.round(estKw), capacityFactor: r2(cf) } };
    },
  },
  {
  ...
  {
    id: "air-quality-ops", name: "Air-Quality Operations Gate", emoji: "😷", sector: "Logistics / HSE",
    inputs: ["pm25", "pm10"],
    decide(o) {
      const pm25 = o.pm25, pm10 = o.pm10;
      let signal, action, score;
      if (pm25 >= 75 || pm10 >= 150) { signal = "HAZARD"; action = "HALT (실외 작업 중단·마스크 의무)"; score = 1; }
      else if (pm25 >= 35 || pm10 >= 80) { signal = "UNHEALTHY"; action = "LIMIT (노출 시간 제한·민감군 보호)"; score = 0.6; }
      else { signal = "GOOD"; action = "GO (정상 운영)"; score = clamp01(pm25 / 75); }
      return { signal, action, score: r2(score), rationale: `PM2.5 ${pm25} ㎍/㎥ · PM10 ${pm10} ㎍/㎥.`, metrics: { pm25, pm10 } };
    },
  },
  {
    id: "uv-advisory", name: "UV Exposure Advisory", emoji: "☀️", sector: "Health / Retail",
    inputs: ["uvIndex"],
    decide(o) {
      const uv = o.uvIndex;
      let signal, action, score;
      if (uv >= 11) { signal = "EXTREME"; action = "AVOID (실외 노출 회피·자외선차단 필수)"; score = 1; }
      else if (uv >= 8) { signal = "VERY_HIGH"; action = "PROTECT (차단제·그늘·보호장구)"; score = 0.8; }
      else if (uv >= 6) { signal = "HIGH"; action = "PROTECT (차단제 권장)"; score = 0.6; }
      else if (uv >= 3) { signal = "MODERATE"; action = "CAUTION (점심시간 주의)"; score = 0.4; }
      else { signal = "LOW"; action = "OK (정상)"; score = clamp01(uv / 11); }
      return { signal, action, score: r2(score), rationale: `UV 지수 ${r2(uv)}.`, metrics: { uvIndex: r2(uv) } };
    },
  },
  {
  ...
  },
  {
    ...
    inputs: ["temperature", "discomfortIndex", "uvIndex", "precipitation"],
    decide(o) {
      const t = o.temperature, di = o.discomfortIndex, uv = o.uvIndex, p = o.precipitation || 0;
      // comfort 100 best: penalize temp away from 23°C, high discomfort, high UV, rain
      let c = 100 - Math.abs(t - 23) * 3 - Math.max(0, di - 70) * 2 - Math.max(0, uv - 6) * 3 - Math.min(40, p * 8);
      ...
      return { signal, action, score: r2(c / 100), rationale: `쾌적도 ${c}/100 (기온 ${r2(t)}℃·불쾌 ${r2(di)}·UV ${r2(uv)}·강수 ${r2(p)}mm).`, metrics: { comfortScore: c, temperature: r2(t), discomfortIndex: r2(di), uvIndex: r2(uv) } };
    },
  },
  {
