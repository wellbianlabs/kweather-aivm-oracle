...
// id; fall back to (network, asset); finally the first asset (legacy clients).
function matchRequirement(accepts, payment) {
  if (payment.id) {
    const byId = accepts.find((r) => r.extra && r.extra.id === payment.id);
    if (byId) return byId;
  }
...
module.exports = { PERMIT2, DEFAULTS, assetCatalogue, settlerAddress, buildAccepts, matchRequirement, settle, chainIdFor, explorerFor, rpcFor };
